"""PTT Lifeismoney 版爬蟲 + LLM 抽取入庫組裝入口（F21）。

``python -m rag.ptt_ingest`` 手動執行：爬列表頁近 N 頁 → 逐篇抓內文 → LLM 抽取 →
通過 schema 驗證才入庫。單篇失敗只記 log 跳過，不中斷整批（設計比照 rag/backfill.py）。

這一輪不掛進 scraper/runner.py 的固定跑批清單（見 feature_list.json F21 的
scope_note：正式接線入口留待下一輪，此輪先確立 scraper 純爬取／rag 側抽取入庫的分工）。
"""
...
from scraper.sources.lifeismoney import build_client, fetch_recent_article_urls, parse_article
...
def ingest_lifeismoney(
    ...
    for url in urls:
        try:
            ...
            logger.warning("PTT 文章抓取失敗，跳過：%s（%s）", url, error)
...
def main() -> int:
    ...
    try:
        result = ingest_lifeismoney(conn, build_client().get_text, complete, datetime.now())
    finally:
        conn.close()
